...
{
    public class VrLobbyManager : NetworkLobbyManager
    {
        static short MsgKicked = MsgType.Highest + 1;

        static public VrLobbyManager s_Singleton;
        ...
        */
        ...
        public override GameObject OnLobbyServerCreateLobbyPlayer(NetworkConnection conn, short playerControllerId)
        {
            ...
            VrLobbyPlayer newPlayer = obj.GetComponent<VrLobbyPlayer>();
            ...
            {
                VrLobbyPlayer p = lobbySlots[i] as VrLobbyPlayer;
        ...
        public IEnumerator ServerCountdownCoroutine()
        {
                ...
                {//to avoid flooding the network of message, we only send a notice to client when the number of plain seconds change.
                    ...
                    for (int i = 0; i < lobbySlots.Length; ++i)
                    {
                        ...
                        {//there is maxPlayer slots, so some could be == null, need to test it before accessing!
                            (lobbySlots[i] as VrLobbyPlayer).RpcUpdateCountdown(floorTime);
                        }
                    }
                }
            }
            ...
            for (int i = 0; i < lobbySlots.Length; ++i)
            {
                if (lobbySlots[i] != null)
                {
                    (lobbySlots[i] as VrLobbyPlayer).RpcUpdateCountdown(0);
                }
            }
